...
프로젝트에는 GitHub 협업 자동화를 위한 `.github` 설정이 포함되어 있습니다.

`.github` 폴더는 코드 실행용이 아니라 GitHub 협업 및 CI 자동화를 위한 설정 폴더입니다.
...
- `.github/workflows/ci.yml`
  ...
  - PR 생성 또는 `main` 브랜치 push 시 자동으로 검사 작업을 실행합니다.
...
이를 통해 lint 오류, 타입 오류, 테스트 실패 등을 PR 단계에서 자동으로 검증할 수 있습니다.

현재 `npm run lint`는 모바일 ESLint와 백엔드 Checkstyle을 함께 실행합니다.
...
- `npm run lint`: 모바일 ESLint와 백엔드 Checkstyle을 함께 실행
- `npm run typecheck`: 모바일 TypeScript 타입 검사 실행
- `npm run test`: 백엔드 테스트 실행
- `npm run check`: lint, typecheck, test를 한 번에 실행
